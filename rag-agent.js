...
class OptimizedRAGAgent {
  ...
  splitText(text, chunkSize = this.maxChunkSize, overlap = this.chunkOverlap) {
    ...
    if (overlap >= chunkSize) {
      console.warn("overlap should be less than chunkSize, adjusting...");
      overlap = Math.floor(chunkSize * 0.1); // 设为chunkSize的10%
    }

    const chunks = [];
    let start = 0;
    let iterationCount = 0; // 防护计数器
    ...
    if (iterationCount >= maxIterations) {
      console.error("splitText: 检测到潜在的无限循环，已强制退出");
    }
  ...
  async addDocuments(documents) {
    ...
    for (let i = 0; i < documents.length; i += this.batchSize) {
      ...
      console.log(
        `处理批次 ${Math.floor(i / this.batchSize) + 1}/${Math.ceil(
          documents.length / this.batchSize
        )}`
      );
    ...
    for (const filePath of filePaths) {
      try {
        ...
        if (stats.size > maxFileSize) {
          console.warn(
            `文件 ${filePath} 太大 (${(stats.size / 1024 / 1024).toFixed(
              2
            )}MB)，跳过`
          );
          continue;
        ...
        console.log(
          `加载文件: ${filePath} (${(stats.size / 1024).toFixed(1)}KB)`
        );
      } catch (error) {
